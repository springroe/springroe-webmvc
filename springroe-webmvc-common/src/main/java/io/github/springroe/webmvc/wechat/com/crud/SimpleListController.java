 ...
 */
public interface SimpleListController<T extends Persistable<ID>, ID extends Serializable, D> extends EntityController<T, ID> {

    @Operation(summary = "List")
    @PostMapping(path = "/list/simple")
    // @SerializationFilter(templateMethodName = "listSerializationFilter")
    // @PreAuthorize("@ss.hasPermi('query')")
    default ApiResult<Iterable<D>> simpleList(@RequestBody List<ConditionParam> conditions) {
